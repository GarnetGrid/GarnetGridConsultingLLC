...
**Common mistakes**
- Using `MIN('FactTable'[FactDateColumn])` directly (works only in certain contexts; MINX/MAXX over ALL is safer).
- Forgetting to **Mark as date table** (Model view → select DateTable → Mark as date table).
- Making relationships bi-directional by default (usually keep single direction from DateTable to Fact).
...
- All measures should use DateTable for filtering time periods
