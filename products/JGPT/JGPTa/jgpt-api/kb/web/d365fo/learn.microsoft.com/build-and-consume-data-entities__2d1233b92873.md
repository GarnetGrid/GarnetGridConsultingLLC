...
**Source:** https://learn.microsoft.com/en-us/dynamics365/fin-ops-core/dev-itpro/data-entities/build-consuming-data-entities
**Captured:** 2026-02-02
**Domain:** d365fo
...
* Create your own package and model. For more information, see [Models and packages](../dev-tools/models).
...
* On a [local virtual machine (VM)](../dev-tools/access-instances#vm-that-is-running-locally), the base URL is `https://usnconeboxax1aos.cloud.onebox.dynamics.com`.
...
2. Right-click the project, select **Properties**, and verify that the project is in the Fleet Management model. If it isn't, set the **Model** property to **Fleet Management**.
...
4. In the **Data entity** wizard, specify the properties for the data entity that you're creating. Use the values that are shown in the following image.
   ...
   ![Screenshot of Data Entity Wizard.](media/data-entity-wizard.png)
5. Select **Next**. For more information about the function of each property, see "Categories of entities" and "Building an entity" in [Data entities overview](data-entities).
...
7. Rename the data entity fields to reflect public data contract standards, or select **Convert labels to field names** to generate names from the existing labels.
8. On the line for the **DriversLicense** field, select the **Is mandatory** check box. This field is used as the natural key for the entity.

   ![Screenshot of Data Entity Wizard 2.](media/data-entity-wizard-2.png)
9. In the **Data source** field, select **PrimaryAddress**. Notice that the **PrimaryAddress** data source is automatically added because of automatic expansion or the surrogate foreign key replacement of **AddressID**.
   ...
   ![Screenshot of steps and add fields.](media/steps-and-add-fields.png)
10. Select the fields from the **PrimaryAddress** data source that you want to be part of your entity. Additionally, rename the following fields to reflect proper public data contract naming:
    ...
    * PrimaryAddress \_Country > Country
    ...
    ![Screenshot of Data entity wizard.](media/data-entity-wizard-3.png)
11. Select **Finish**. A data entity item and staging table are added to the project.

    ![Screenshot of Solution explorer.](media/solution-explorer.png)

### Build your project

1. In Solution Explorer, right-click your project, and then select **Properties**.
2. Change the value of the **Synchronize database on build** property to **True**, and then select **OK**. This property must be set only one time per project.
   ...
   ![Screenshot of Property pages.](media/property-pages.png)
3. On the Visual Studio toolbar, Select **Build** > **Build Solution** to build the project.
...
3. Set the **Label** property to **Fleet Lab Customers**.
   ...
   ![Screenshot of FMLabCustomerEntity - Properties.](media/fmlabcustomerentity-properties.png)
4. In the left pane, select **Data Sources** > **FMCustomer** > **Data Sources** > **FMAddressTable**.
5. Change the **Is Read Only** property to **No**. This is a known issue. Eventually, the value is set to **Yes** or **No** automatically, based on the type of join. The value should be **Yes** for composition scenarios, and **No** for associations (surrogate foreign key expansions). This property enables the data source to be read/write.
   ...
   ![Screenshot of FMLabCustomerEntity - Properties2.](media/fmlabcustomerentity-properties2.png)
6. In the **FMLabCustomerEntity** designer, select **Keys** > **EntityKey**, and then expand the **Fields** node. Verify that the list of fields matches the following image.

   ![Screenshot of FMLabCustomerEntity.](media/fmlabcustomerentity.png)
7. To visually validate the staging table that will be used for import/export, open the **FMLabCustomerStaging** table in the designer, and then select the **FMLabCustomerStaging** node.
   ...
   ![Screenshot of fMLabCustomerStaging - Properties.](media/fmlabcustomerstaging-properties.png)
8. Select **FMLabCustomerStaging** > **Fields**. In the following image, the standard fields of the staging tables are selected. All entity staging tables have these standard fields. The image also shows the data fields that belong on this data entity.

   ![Screenshot of FMLabCustomerStaging.](media/fmlabcustomerstaging.png)
...
2. Copy and paste the following code into the class to test your data entity.
   ...
   ```
3. Run the code in the debugger to set it as a startup object.
4. To validate the entity, view the Infolog in the debugger window or in notifications on the website. You see that three successful messages are logged. You also see the actions that were taken.
...
1. Create a sample CSV file that you can import. Copy the following text and save it as **FM Lab Customer Import.csv**.
   ...
   ```
2. Select **User Dashboard** > **Data management**.
3. In the **Data Management** workspace, select the **Import** tile.
4. On the **Import** page, enter the import details as shown in the following image.

   ![Screenshot of Import new record.](media/import-new-record.png)
5. Select the **Upload data** button next to the **Upload file for entity** field, and select the CSV file that you created.
6. After the file is uploaded, you notice that the entity is added to the middle section. You also receive an error that states that the mapping isn't valid. A few fields aren't mapped correctly between the source file and the target entity.
7. In the entities list, select **View map**.
8. **AddressLine1** and **AddressLine2** are two fields in the source that aren't mapped to target fields. In the visual mapper or details view, map these fields as follows, and then select **Save**:

   * AddressLine1 – Address1
   * AddressLine2 – Address2

   ![Screenshot of Map source to staging.](media/map-source-to-staging.png)
9. Select the **Back** button in your browser to go back to the **Import job** page. The check mark in the entities list indicates that the entity is now ready for import.

   ![Screenshot of Import new record 2.](media/import-new-record-2.png)
10. Select **Import Now**. After the import is completed, the job status page opens.
...
7. Select **Customer Relation**, and then select **Delete**.
...
12. Set the following properties.
    ...
    ![Screenshot of FMRentalEntity - Solution explorer.](media/fmrentalentity-solution-explorer.png)
13. On the **BUILD** menu, select **Build Solution** to save your changes and build the project. You can view the build progress in the **Output** window.
...
3. In the **Registry Editor** dialog box, select **Yes**.
4. Select **OK**.
...
2. In the browser, find **FleetRental**. The following image shows the metadata of the **FleetRental** entity, together with the new relationship, **NavigationProperty**.
   ...
   ![Screenshot of FleetRental metadata.](media/fleetrental-metadata.png)
...
4. If you don't want to retrieve all properties for the customers, you can retrieve selected properties. For example, to retrieve only **FirstName** and **LastName**, enter the following URL: [baseURL]/data/FleetCustomers?$filter=FirstName.LastName
...
2. Copy the following code lines, and paste them into the **Code** window.
   ...
   ```
3. On the **BUILD** menu, select **Rebuild Solution** to save your changes and build the project. You can view the build progress in the **Output** window.
...
4. Select **Open**. The **Odata4ConsoleApplication** project appears in Solution Explorer.
...
8. In the **Save of Read-only file** dialog box, select **Overwrite**. The proxy class for the OData metadata endpoint is generated. This operation might take a few minutes.
...
11. Verify that there's a final closing slash (/) in the URL, and then select **Save**.
12. In the **Save of Read-only file** dialog box, select **Overwrite**.
...
14. Follow these steps to verify that the new reservation appears on the **Rental** page:
    ...
    1. Start Internet Explorer, and enter the following URL in the address bar: [baseURL]/?mi=FMRental The **FMRental** page shows the list of rentals.
       ...
       ![Screenshot of Rental list.](media/rental-list.png)
    2. At the bottom of the list, select **Next** to view the next page. On this page, you can see that the reservation was created for the new customer that you added.

       ![Screenshot of customer reservation.](media/customer-reservation.png)
...
[Develop entities for data migration](develop-entity-for-data-migration)
