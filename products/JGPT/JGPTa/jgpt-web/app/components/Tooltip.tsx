...
export default function Tooltip({ content, children, position = 'bottom', className = '' }: TooltipProps) {
    ...
    return (
        <div className={`relative group inline-block ${className}`}>
            {children}
            <div className={`absolute ${positionClasses[position]} z-50 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 pointer-events-none`}>
                <div className="bg-gray-900 text-white text-xs rounded-lg px-3 py-2 whitespace-nowrap shadow-xl border border-white/10">
                    {content}
                    <div className={`absolute ${arrowClasses[position]} w-0 h-0 border-4`}></div>
                </div>
            </div>
        </div>
    );
